...
export async function POST(
  ...
  try {
    ...
    if (!ensureApproverRole(session)) {
      return errorResponse("Insufficient permissions to reject compensation profiles", 403)
    }
    ...
    if (!canTransitionStandardWorkflow(existing.workflowStatus, "REJECT")) {
      return errorResponse("Only submitted profiles can be rejected", 400)
    }
    ...
    console.error("[API] POST /api/compensation/profiles/[id]/reject error:", error)
    return errorResponse("Failed to reject compensation profile")
  }
}
