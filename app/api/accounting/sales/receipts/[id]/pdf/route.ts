...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const record = await prisma.salesReceipt.findUnique({
      where: { id },
    ...
    console.error("[API] GET /api/accounting/sales/receipts/[id]/pdf/route.ts error:", error);
    return errorResponse("Failed to render Receipt PDF");
  }
}
