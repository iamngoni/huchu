...
import { Alert, Button, Card, EmptyState, Select, Skeleton } from "@corelithzw/react";
...
export function TeacherMarksScreen() {
  ...
  return (
    <div className="flex flex-col gap-4">
      {list.error ? (
        <Alert tone="danger" title="Your assessments would not load">
          {getApiErrorMessage(list.error)}
        </Alert>
      ) : null}
      {save.error ? (
        <Alert tone="danger" title="The marks did not save">
          {getApiErrorMessage(save.error)}
        </Alert>
      ) : null}
      {saved ? <Alert tone="success" title={saved} onDismiss={() => setSaved(null)} /> : null}

      {!list.isPending && assessments.length === 0 ? (
        <EmptyState
          ...
          >
            {sheet.isPending ? (
              ...
              <EmptyState
                ...
                body="No active pupil has this class as their year group, so there is nothing to mark."
              />
            ) : (
